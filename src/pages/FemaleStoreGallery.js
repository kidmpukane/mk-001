...
const FemaleStoreGallery = (props) => {
  const { item } = props.route.params;
  const parsedItem = JSON.parse(item); // Parse the item here
  const navigation = useNavigation();
  const storeInfoUrl = `http://10.0.2.2:8000/api/products/primary-products-for-collections/${parsedItem?.id}/`;
